...
describe('useEpgNavigation', () => {
  ...
  it('initializes cursor at initialIndex', () => {
    const { result } = renderHook(() =>
      useEpgNavigation({ isOpen: true, channelCount: 5, initialIndex: 2, onSelect, onClose }),
    )
  ...
  it('ArrowDown moves cursor down', () => {
    const { result } = renderHook(() =>
      useEpgNavigation({ isOpen: true, channelCount: 5, initialIndex: 1, onSelect, onClose }),
    )
  ...
  it('ArrowUp moves cursor up', () => {
    const { result } = renderHook(() =>
      useEpgNavigation({ isOpen: true, channelCount: 5, initialIndex: 3, onSelect, onClose }),
    )
    act(() => { fireKey('ArrowUp') })
    expect(result.current.cursorIndex).toBe(2)
  })
  ...
  it('ArrowDown wraps from last to first', () => {
    const { result } = renderHook(() =>
      useEpgNavigation({ isOpen: true, channelCount: 5, initialIndex: 4, onSelect, onClose }),
    )
    act(() => { fireKey('ArrowDown') })
  ...
  it('ArrowUp wraps from first to last', () => {
    const { result } = renderHook(() =>
      useEpgNavigation({ isOpen: true, channelCount: 5, initialIndex: 0, onSelect, onClose }),
    )
    act(() => { fireKey('ArrowUp') })
  ...
  it('does not respond to keys when isOpen is false', () => {
    renderHook(() =>
      useEpgNavigation({ isOpen: false, channelCount: 5, initialIndex: 2, onSelect, onClose }),
    )
    act(() => { fireKey('ArrowDown') })
