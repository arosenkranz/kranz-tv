import type { ChannelPreset } from './types.ts'

export const CHANNEL_PRESETS: readonly ChannelPreset[] = [
  {
    id: 'skate',
    number: 1,
    name: 'Skate Vids',
    description: 'Skateboarding clips and edits',
    kind: 'video' as const,
    playlistId: 'PLmDOmgjgiHsiBYTWTmljl4E3Ft0DBVlDH',
  },
  {
    id: 'music',
    number: 2,
    name: 'Music Videos',
    description: 'Music videos from the collection',
    kind: 'video' as const,
    playlistId: 'PLmDOmgjgiHshjqFKDhoSFjVLtyVOnOaOn',
  },
  {
    id: 'party',
    number: 3,
    name: 'Party Background',
    description: 'Background vibes for any occasion',
    kind: 'video' as const,
    playlistId: 'PLmDOmgjgiHsg9L_50qUKKeTYa3CR33o9s',
  },
  {
    id: 'favorites',
    number: 4,
    name: 'Favorites',
    description: 'All-time favorite videos',
    kind: 'video' as const,
    playlistId: 'FLRbSMOWF7L-IrYVrZ0qBRfw',
  },
  {
    id: 'entertainment',
    number: 5,
    name: 'Entertainment',
    description: 'Entertainment picks from around the web',
    kind: 'video' as const,
    playlistId: 'PLmDOmgjgiHsgXkkeyJM1E8-Lx28ds32Nt',
  },
  {
    id: 'club-krunz',
    number: 6,
    name: 'Club Krünz',
    description: 'The club never closes',
    kind: 'video' as const,
    playlistId: 'PLmDOmgjgiHsjM5E1QSC1BTb7mE6YsOL0Z',
  },
  {
    id: 'live-music',
    number: 7,
    name: 'Live Music',
    description: 'Live performances and concert footage',
    kind: 'video' as const,
    playlistId: 'PLmDOmgjgiHsgOwvlmX382R3mfmghM5BZ3',
  },
  {
    id: 'electronic-mixes',
    number: 8,
    name: 'Mixes and Sets',
    description: 'DJ mixes and extended sets',
    kind: 'video' as const,
    playlistId: 'PLmDOmgjgiHsjFc_P7McjUhcELpscfWoag',
  },
  {
    id: 'slow',
    number: 9,
    name: 'Slow',
    description: 'Slow and steady',
    kind: 'video' as const,
    playlistId: 'PLmDOmgjgiHsijEtzJ4WhbwchG_fTU1e_4',
  },
  {
    id: 'patagonia-surfing',
    number: 10,
    name: 'Patagonia | Surfing',
    description: 'Surfing in Patagonia',
    kind: 'video' as const,
    playlistId: 'PL58D86239D8EE4A6A',
  },
  {
    id: 'radio-soulwax',
    number: 11,
    name: 'Radio Soulwax',
    description: 'Radio Soulwax broadcasts',
    kind: 'video' as const,
    playlistId: 'PL5PnoBesS3Uyv2mgkgCnEBhOeD0YE97OO',
  },
  {
    id: 'funny',
    number: 12,
    name: 'Funny',
    description: 'Comedy clips and funny videos',
    kind: 'video' as const,
    playlistId: 'PLmDOmgjgiHsg1V4vIR7XhgheeB-PT1KtU',
  },
  {
    id: 'monsterpiece-animation',
    number: 13,
    name: 'Monsterpiece Theatre — Animation',
    description: 'Animated shorts from Monsterpiece Theatre',
    kind: 'video' as const,
    playlistId: 'PL9vpEzfAbupVx53xfthK53R3vUQBhrhNq',
  },
  {
    id: 'monsterpiece-muppets',
    number: 14,
    name: 'Monsterpiece Theatre — Muppets',
    description: 'Muppet sketches from Monsterpiece Theatre',
    kind: 'video' as const,
    playlistId: 'PL9vpEzfAbupUxDepvdwVdsJqU2imuydl_',
  },
  {
    id: 'monsterpiece-international',
    number: 15,
    name: 'Monsterpiece Theatre — International',
    description: 'International shorts from Monsterpiece Theatre',
    kind: 'video' as const,
    playlistId: 'PL9vpEzfAbupWjuUMuLpmb54lNxbgEMGWN',
  },
  {
    id: 'bumbershoot-gardyloo',
    number: 16,
    name: 'Bumbershoot Gardyloo',
    description: 'Bumbershoot festival performances',
    kind: 'video' as const,
    playlistId: 'PLQxh1ekjHJ2UNYqrSzC-lJ4j6ZjgtaiF8',
  },
  {
    id: 'caribou-mixtape',
    number: 17,
    name: 'Caribou Mixtape',
    description: 'Caribou — The Longest Mixtape',
    kind: 'video' as const,
    playlistId: 'PL4lEESSgxM_5O81EvKCmBIm_JT5Q7JeaI',
  },
  {
    id: 'mlb-classic-games',
    number: 18,
    name: 'MLB Classic Games',
    description: 'Classic MLB games in full',
    kind: 'video' as const,
    playlistId: 'PLi70flcQkwNvxC7kKFsUKKaH810CZlRfW',
  },
  {
    id: 'mlb-full-games',
    number: 19,
    name: 'MLB Full Games',
    description: 'Full-length MLB games',
    kind: 'video' as const,
    playlistId: 'PLL-lmlkrmJalUXp2fcWbcyZEWfxYvzTpD',
  },
  {
    id: 'wwe-at-msg',
    number: 20,
    name: 'WWE at MSG',
    description: 'WWE events at Madison Square Garden',
    kind: 'video' as const,
    playlistId: 'PLS9U3H1hd2sw',
  },
  {
    id: 'thats-delicious',
    number: 21,
    name: "F**k That's Delicious",
    description: "Action Bronson's food and travel show",
    kind: 'video' as const,
    playlistId: 'PLSj8CHKXkPzIMtw5pVjDz1wsQpJeOnyoo',
  },
  {
    id: 'epicly-laterd',
    number: 22,
    name: "Epicly Later'd",
    description: 'VBS skateboarding documentary series',
    kind: 'video' as const,
    playlistId: 'PLDbSvEZka6GGm9cigiCJImOCKj6XZ9-gY',
  },
  {
    id: 'sc-calming',
    number: 23,
    name: 'Calming',
    description: 'Calming SoundCloud set',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/calming',
  },
  {
    id: 'sc-all-time-favorites',
    number: 24,
    name: 'All Time Favorites',
    description: 'Krünz all-time favorite tracks',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/all-time-favorites',
  },
  {
    id: 'sc-strato-krunz',
    number: 25,
    name: 'Strato & Krünz',
    description: 'Strato & Krünz collaborations',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/strato-kr-nz',
  },
  {
    id: 'sc-eclectic',
    number: 26,
    name: 'Eclectic',
    description: 'Eclectic SoundCloud set',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/eclectic',
  },
  {
    id: 'sc-deeply-disco',
    number: 27,
    name: 'Deeply Disco',
    description: 'Deep disco grooves',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/deep-disco',
  },
  {
    id: 'sc-2016-sets',
    number: 28,
    name: '2016 Sets',
    description: 'Sets from 2016',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/2016-sets',
  },
  {
    id: 'sc-sets-2020',
    number: 29,
    name: 'Sets - 2020',
    description: 'Sets from 2020',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/sets-2020',
  },
  {
    id: 'sc-sets-2019',
    number: 30,
    name: 'Sets - 2019',
    description: 'Sets from 2019',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/sets-2019',
  },
  {
    id: 'sc-sets-2018',
    number: 31,
    name: 'Sets - 2018',
    description: 'Sets from 2018',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/sets-2018',
  },
  {
    id: 'sc-sets-2017',
    number: 32,
    name: 'Sets - 2017',
    description: 'Sets from 2017',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/2017-sets-1',
  },
  {
    id: 'sc-funk-disco-yacht',
    number: 33,
    name: 'Funk/Disco/Yacht',
    description: 'Funk, disco & yacht rock selections',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/funk-disco-yacht',
  },
  {
    id: 'sc-deeply-disco-deux',
    number: 34,
    name: 'Deeply Disco Deux',
    description: 'Deep disco grooves, part two',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/deeply-disco-duex',
  },
  {
    id: 'sc-sets-june-2015',
    number: 35,
    name: 'Sets - June 2015',
    description: 'Sets from June 2015',
    kind: 'music' as const,
    source: 'soundcloud' as const,
    sourceUrl: 'https://soundcloud.com/krunz/sets/sets-june-15',
  },
] as const
